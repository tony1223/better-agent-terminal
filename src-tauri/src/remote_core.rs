...
fn legacy_v1_param_keys(channel: &str) -> Option<&'static [&'static str]> {
    ...
    match canonical.as_str() {
        ...
        "claude:scan-mcp-servers" => Some(&["cwd"]),
        "claude:get-mcp-server-status" => Some(&["sessionId"]),
        "claude:reconnect-mcp-server" => Some(&["sessionId", "name"]),
        "claude:toggle-mcp-server" => Some(&["sessionId", "name", "enabled"]),
        "claude:set-mcp-server-enabled" => Some(&["cwd", "name", "enabled", "source"]),
